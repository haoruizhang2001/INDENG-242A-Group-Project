...
---
Below are the specific definitions for each model
...
This model assumes the market is efficient and the best predictor for next week's price is simply this week's price.
...
This model assumes the current price might contain noise, so it uses the average of the last two weeks as the prediction anchor.

$$\hat{P}_{t+1} = \frac{P_t + P_{t-1}}{2}$$

## 4.  Supply/Demand Calibrated Random Walk (Augmented)

This model uses the Random Walk as a base ($P_t$) but "calibrates" it by predicting the percentage return ($\hat{r}_{t+1}$) based on changes in supply and demand fundamentals.

$$\hat{r}_{t+1} = \alpha + \sum_{i=1}^{k} \lambda_i \Delta F_{i,t}$$
